...
export function loadConfig(): ParserConfig {
  ...
  return {
    ...
    pollIntervalMs: Number.isFinite(pollIntervalMs) ? pollIntervalMs : DEFAULTS.POLL_INTERVAL_MS,
    pollJitterMs: Number.isFinite(pollJitterMs) ? pollJitterMs : DEFAULTS.POLL_JITTER_MS,
    kmaTimeoutMs: Number.isFinite(kmaTimeoutMs) ? kmaTimeoutMs : DEFAULTS.KMA_TIMEOUT_MS,
    postTimeoutMs: Number.isFinite(postTimeoutMs) ? postTimeoutMs : DEFAULTS.POST_TIMEOUT_MS,
    postRetries: Number.isFinite(postRetries) ? postRetries : DEFAULTS.POST_RETRIES,
