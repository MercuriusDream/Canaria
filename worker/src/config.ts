...
export class ConfigManager {
  ...
  update(partial: Partial<Config>): void {
    ...
    if (partial.rateLimit) {
      ...
      if (partial.rateLimit.limits) {
        this.cache.rateLimit.limits = { ...this.cache.rateLimit.limits, ...partial.rateLimit.limits };
      }
    }
    if (partial.monitoring) {
      this.cache.monitoring = { ...this.cache.monitoring, ...partial.monitoring };
    }
    this.persist();
  }

  getRollupIntervalMs(): number {
    const intervals = { "1m": 60_000, "5m": 300_000, "15m": 900_000, "1h": 3_600_000 };
  ...
  getRateLimit(endpoint: string): { maxRequests: number; windowSeconds: number } | null {
  ...
  private loadOrInitialize(envVars?: Record<string, string | undefined>): Config {
    const row = this.db.prepare("SELECT value FROM config WHERE key = ?").first("app_config");
    ...
    if (envVars) {
      ...
      if (Number.isFinite(rollupRetention) && rollupRetention >= 1 && rollupRetention <= 365) {
        config.metrics.rollupRetentionDays = rollupRetention;
      }

      if (envVars.RATE_LIMIT_ENABLED) {
        config.rateLimit.enabled = envVars.RATE_LIMIT_ENABLED.toLowerCase() === "true";
      }
    }
