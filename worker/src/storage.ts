import { ListQuery, EventRecord } from "./types";
...
export class EventStore {
  ...
  private ensureTables(): void {
    ...
    this.db.exec(`CREATE INDEX IF NOT EXISTS idx_metrics_rollup_timestamp ON metrics_rollup(timestamp)`);
    this.db.exec(`CREATE INDEX IF NOT EXISTS idx_feed_events_timestamp ON feed_events(timestamp)`);
  }
  ...
  latest(): EventRecord | null {
    const row = this.db.prepare("SELECT * FROM events ORDER BY time DESC LIMIT 1").first();
  ...
  countBySource(source: string): number {
    const row = this.db.prepare("SELECT COUNT(1) as c FROM events WHERE source = ?").first(source);
    if (row && typeof row.c === "number") {
      return row.c;
    }
    return 0;
  ...
  getOldestEvent(): EventRecord | null {
    const row = this.db.prepare("SELECT * FROM events ORDER BY time ASC LIMIT 1").first();
    return row ? this.deserialize(row) : null;
  }

  getTableStats(): { events: number; metrics_rollup: number; request_logs: number; rate_limits: number } {
    ...
    return stats as { events: number; metrics_rollup: number; request_logs: number; rate_limits: number };
  }

  deleteOldEvents(daysOld: number): number {
    const cutoff = new Date(Date.now() - daysOld * 24 * 60 * 60 * 1000).toISOString();
    const result = this.db.prepare("DELETE FROM events WHERE time < ?").run(cutoff) as
      | { changes?: number }
      | void;
    return result && "changes" in result && typeof result.changes === "number" ? result.changes : 0;
  }
  ...
  private deserialize(row: Record<string, unknown>): EventRecord {
    return {
      ...
      reportType: typeof row.reportType === "number" || typeof row.reportType === "string" ? row.reportType : null,
      time: String(row.time),
      issueTime: typeof row.issueTime === "string" ? row.issueTime : null,
      receiveTime: typeof row.receiveTime === "string" ? row.receiveTime : String(row.time),
      receiveSource: typeof row.receiveSource === "string" ? row.receiveSource : "unknown",
