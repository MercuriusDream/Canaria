...
export class RateLimiter {
  ...
  check(ip: string, endpoint: string): RateLimitResult {
    ...
    const row = this.db.prepare("SELECT count, window_start FROM rate_limits WHERE key = ?").first(key);

    let count = 0;
    if (row && typeof row.count === "number" && typeof row.window_start === "number") {
  ...
  reset(ip: string, endpoint?: string): void {
    if (endpoint) {
      this.db.prepare("DELETE FROM rate_limits WHERE key = ?").run(`${ip}:${endpoint}`);
    } else {
      this.db.prepare("DELETE FROM rate_limits WHERE key LIKE ?").run(`${ip}:%`);
    }
  }
  ...
  getTopIPs(limit: number = 10): { ip: string; requests: number; blocked: number }[] {
    const rows = this.db
      .prepare(
        `SELECT
