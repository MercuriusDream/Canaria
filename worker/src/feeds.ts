...
import { EventRecord, FeedState } from "./types";
...
export class FeedManager {
  ...
  private handleWolfxMessage(data: any): void {
...
class WebSocketConnector {
  ...
  constructor(
    private readonly name: "wolfx" | "p2p",
  ...
  private connect(): void {
    try {
      ...
      ws.addEventListener("message", (event) => {
        const now = new Date().toISOString();
        this.updateState({ lastMessage: now, lastHeartbeat: now, lastError: null });
        this.resetInactivityTimer();
        try {
      ...
      this.updateState({ status: "disconnected", lastError: stringifyError(error) });
  ...
  private handleClose(reason: string): void {
    ...
    if (this.state.connectedAt && this.state.status === "connected") {
      const sessionUptime = Date.now() - new Date(this.state.connectedAt).getTime();
