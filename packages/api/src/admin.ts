...
export class AdminHandler {
  ...
  getDashboard(
    ...
    const recentEventCount = this.db
      .prepare(
        "SELECT COUNT(*) as c FROM events WHERE CAST((julianday(time) - 2440587.5) * 86400.0 AS INTEGER) >= ?",
      )
      .first(fiveMinutesAgo);
    const eventsPerMinute =
      recentEventCount && typeof recentEventCount.c === "number"
        ? recentEventCount.c / 5
        : 0;

    const wsHistory = this.db
      .prepare(
        "SELECT timestamp, count FROM ws_client_history ORDER BY timestamp DESC LIMIT 60",
      )
      .all();
    ...
    const wolfxEvents = this.db
      .prepare(
        "SELECT event FROM feed_events WHERE feed = 'wolfx' AND timestamp >= ?",
      )
      .all(twentyFourHoursAgo);
    const p2pEvents = this.db
      .prepare(
        "SELECT event FROM feed_events WHERE feed = 'p2p' AND timestamp >= ?",
      )
      .all(twentyFourHoursAgo);

    const wolfxReconnects = wolfxEvents.filter(
      (e) => e.event === "connected",
    ).length;
    const p2pReconnects = p2pEvents.filter(
      (e) => e.event === "connected",
    ).length;

    const wolfxConnectedTime = calculateUptime(
      wolfxEvents.map((e) => String(e.event)),
    );
    const p2pConnectedTime = calculateUptime(
      p2pEvents.map((e) => String(e.event)),
    );

    const topIPs = this.rateLimiter.getTopIPs(10);
    const blockedCount = this.db
      .prepare("SELECT COUNT(*) as c FROM request_logs WHERE status = 429")
      .first();
    const totalBlocked =
      blockedCount && typeof blockedCount.c === "number" ? blockedCount.c : 0;
    ...
    return {
      system: {
        ...
        lastRollup:
          lastRollup > 0 ? new Date(lastRollup).toISOString() : "Never",
      },
      events: {
        total: totalEvents,
  ...
  handleAction(
    ...
    // biome-ignore lint/suspicious/noExplicitAny: Internal dependency
    feedManager?: any,
  ): AdminActionResponse {
    try {
      switch (action.action) {
        case "reconnect_feed":
          if (!action.params?.feed)
            return { success: false, message: "Feed parameter required" };
          if (feedManager && typeof feedManager.reconnect === "function") {
            feedManager.reconnect(action.params.feed);
            return {
              success: true,
              message: `Reconnecting ${action.params.feed} feed`,
            };
          }
        ...
        case "clear_old_events": {
          ...
          return {
            success: true,
            message: `Deleted ${deleted} events older than ${daysOld} days`,
            result: { deleted },
          };
        }
        ...
        case "reset_ratelimit":
          if (!action.params?.ip)
            return { success: false, message: "IP parameter required" };
          this.rateLimiter.reset(action.params.ip);
          return {
            success: true,
            message: `Reset rate limit for IP ${action.params.ip}`,
          };
...
function getHeartbeatAge(parserHeartbeat: Heartbeat | null): number {
  ...
  try {
    return Math.floor(
      (Date.now() - new Date(parserHeartbeat.lastParseTime).getTime()) / 1000,
    );
  } catch {
    return Infinity;
  }
...
function estimateDatabaseSize(tableStats: {
  events: number;
