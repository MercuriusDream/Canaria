...
export class HealthMonitor {
  ...
  checkHealth(
    ...
    const parserHealthy =
      getHeartbeatAge(parserHeartbeat) < this.config.getParserTimeoutSeconds();
    const feedsHealthy =
      feedStates.wolfx.status === "connected" ||
      feedStates.p2p.status === "connected";
    ...
    return {
      ...
      sources: {
        KMA: {
          status: kmaStatus,
          lastEvent:
            latestEvent && latestEvent.source === "KMA"
              ? latestEvent.time
              : null,
  ...
  private buildFeedDetails(
    ...
    const uptimePercent =
      systemUptime > 0 ? (totalUptime / systemUptime) * 100 : 0;
  ...
  private buildParserMetrics(
    ...
    const successRate =
      stats.totalParses > 0
        ? (stats.successfulParses / stats.totalParses) * 100
        : 0;
  ...
  private getJMAStatus(feedStates: {
    wolfx: FeedState;
    p2p: FeedState;
  }): "healthy" | "degraded" | "down" {
