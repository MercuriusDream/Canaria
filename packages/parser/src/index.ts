...
import type { Heartbeat, NormalizedEvent, ParserPayload } from "./types";
...
async function pollOnce(): Promise<void> {
  ...
  try {
    const response = await fetchWithTimeout(
      withTimestamp(config.eqkNowUrl),
      config.kmaTimeoutMs,
    );
    if (!response.ok) {
  ...
  const heartbeat: Heartbeat = {
    ...
    stats: {
      ...
      averageDelayMs:
        stats.totalParses > 0 ? stats.totalDelayMs / stats.totalParses : 0,
  ...
  const response = await postJsonWithRetry(
    `${config.workerEndpoint}/v1/events`,
    payload,
    {
  ...
  if (!response) {
    ...
    console.log(
      "[parser] Received sync request from worker, sending buffer (" +
        eventBuffer.length +
        " events)",
    );
    try {
...
async function start(): Promise<void> {
  console.log(
    "[parser] Starting KMA poller with interval",
